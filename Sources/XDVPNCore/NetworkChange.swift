...
/// 由一组接口名算出「物理出口指纹」：滤掉 utun*（VPN 自己的虚拟接口），去重排序后拼接。
/// 指纹变化 = 物理出口集合变了 = 换网 / 漫游到不同接口 / 插拔网线。
/// 必须去重：NWPath.availableInterfaces 会把同一接口报多次（如 IPv4/IPv6 各一条），
/// 且重复次数会随主路径切到 utun 而变化（en0,en0 → en0），不去重会把
/// VPN 全局模式自己加的 def1 路由误判成换网，陷入连接→重建死循环。
